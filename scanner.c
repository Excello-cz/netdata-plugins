...
scanner_print_hdr(const char * name) {
	nd_chart("scannerd", name, "type", "", "", "volume", "scannerd", "type", ND_CHART_TYPE_STACKED);
	...
	nd_chart("scannerd", name, "sc", "", "AntiSPAM Cache", "percentage", "scannerd", "sc", ND_CHART_TYPE_STACKED);
	...
	nd_chart("scannerd", name, "cc", "", "AntiVirus Cache", "percentage", "scannerd", "cc", ND_CHART_TYPE_STACKED);
	/* scanner does not report :CC:0 correctly, therefore we will dclare CC:0
